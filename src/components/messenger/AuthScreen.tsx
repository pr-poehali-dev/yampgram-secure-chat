...
interface AuthScreenProps {
  onAuth: (user: { id: number; name: string; username: string; avatar: string }, token: string) => void;
}
...
export default function AuthScreen({ onAuth }: AuthScreenProps) {
  ...
  const handleRegister = async () => {
    ...
    if (!regUsername.trim() || regUsername.length < 3) { setRegError('Username минимум 3 символа'); return; }
    ...
    try {
      const data = await apiRegister(regUsername, regName, regPassword);
      localStorage.setItem('yamp_token', data.token);
        ...
        <div className="glass rounded-3xl p-6 animate-slide-up" style={{ animationDelay: '0.1s' }}>
          {mode === 'start' && (
            <div className="space-y-4">
              ...
              <p className="text-center text-xs text-muted-foreground mt-2">Никаких ботов и фейковых аккаунтов</p>
            </div>
          )}
          ...
          {mode === 'register' && (
            <div className="space-y-3">
              ...
              <div className="relative">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground"><Icon name="User" size={15} /></div>
                <input type="text" placeholder="Ваше имя" value={regName}
              ...
              <div className="relative">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground"><Icon name="Lock" size={15} /></div>
                <input type={showRegPass ? 'text' : 'password'} placeholder="Пароль (мин. 6 символов)" value={regPassword}
              ...
              <div className="relative">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground"><Icon name="Lock" size={15} /></div>
                <input type={showRegPass ? 'text' : 'password'} placeholder="Повторите пароль" value={regPasswordConfirm}
              ...
              <button onClick={handleRegister} disabled={loading}
                className="w-full py-4 rounded-2xl grad-bg text-white font-semibold glow-purple transition-all hover:scale-[1.02] active:scale-[0.98] mt-1 disabled:opacity-60">
                {loading ? 'Создаём аккаунт...' : 'Зарегистрироваться ⚡'}
              </button>
              <p className="text-center text-xs text-muted-foreground">
                Уже есть аккаунт?{' '}
                <button onClick={() => setMode('login')} className="text-purple-400 hover:text-purple-300 transition-colors">Войти</button>
              </p>
            </div>
          )}
          ...
          {mode === 'login' && (
            <div className="space-y-3">
              ...
              <div className="relative">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground"><Icon name="Lock" size={15} /></div>
                <input type={showLoginPass ? 'text' : 'password'} placeholder="Пароль" value={loginPassword}
              ...
              <button onClick={handleLogin} disabled={loading}
                className="w-full py-4 rounded-2xl grad-bg text-white font-semibold glow-purple transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-60">
                {loading ? 'Входим...' : 'Войти'}
              </button>
              <p className="text-center text-xs text-muted-foreground">
                Нет аккаунта?{' '}
                <button onClick={() => setMode('register')} className="text-purple-400 hover:text-purple-300 transition-colors">Создать</button>
              </p>
            </div>
          )}
        </div>
